...
int countLeafNodes(TreeNode* root) {
	...
	//Base condition to be a leaf node
	if (root->left == NULL && root->right == NULL) {
		return 1;
	}
	int countLeftTreeLeafNodes = countLeafNodes(root->left);
	int countRightTreeLeafNodes = countLeafNodes(root->right);
	return countLeftTreeLeafNodes + countRightTreeLeafNodes;
}
signed main()
{
#ifndef ONLINE_JUDGE
